...
class DashboardViewModel: ObservableObject {
    ...
    func loadDashboard() async {
        ...
        do {
            ...
            s.soldThisMonth = cattleResult.items.filter { $0.status == .sold }.count
            s.recentHealthEvents = healthResult.items
            ...
            let weights = cattleResult.items.compactMap { $0.lastWeight ?? $0.estimatedWeight }
            s.averageWeight = weights.isEmpty ? 0 : weights.reduce(0, +) / Double(weights.count)
