    ...
    <div className="font-body" style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      {purchase && (
        <>
          ...
          <DataTable
            columns={[
              { key: 'earTag', label: 'Arete', render: (c: Cattle) => (
                <Link href={`/cattle/${c.id}`} className="text-primary font-semibold no-underline hover:underline">
                  {c.earTag || `#${c.id}`}
                </Link>
              )},
              { key: 'breed', label: 'Raza', render: (c: Cattle) => c.breed || '—' },
              { key: 'sex', label: 'Sexo', render: (c: Cattle) => c.sex || '—' },
              { key: 'weight', label: 'Peso', render: (c: Cattle) => c.weight != null ? `${c.weight} kg` : '—' },
            ]}
            data={cattle}
